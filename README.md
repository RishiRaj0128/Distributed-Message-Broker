...
> **Infrastructure COE Focus**: Engineered specifically to target high-availability payment infrastructure (Juspay Infrastructure COE domain: Multi-DC Architecture, Declarative Topology Infrastructure DSL, Distributed Partition Consensus, 99.999% Availability Observability, and Split-Brain Chaos Verification).
...
3. [Deep-Dive Design Diagrams (Mermaid.js)](#-deep-dive-design-diagrams-mermaidjs)
   - [3.1 Multi-DC Partition Replication & Quorum ACK Lifecycle](#31-multi-dc-partition-replication--quorum-ack-lifecycle)
   - [3.2 Dynamic In-Sync Replica (ISR) State Machine](#32-dynamic-in-sync-replica-isr-state-machine)
   - [3.3 Partition Failover & Zero-Loss Leader Election](#33-partition-failover--zero-loss-leader-election)
   - [3.4 Declarative Infrastructure DSL Reconciliation Loop](#34-declarative-infrastructure-dsl-reconciliation-loop)
4. [Cross-Project Idempotency & Delivery Semantics](#-cross-project-idempotency--delivery-semantics)
5. [Infrastructure DSL for Unified Topology Management](#-infrastructure-dsl-for-unified-topology-management)
6. [Backpressure & Flow Control](#-backpressure--flow-control)
7. [Observability & Prometheus / Grafana Dashboard](#-observability--prometheus--grafana-dashboard)
8. [Chaos Testing & Split-Brain Invariant Proofs](#-chaos-testing--split-brain-invariant-proofs)
9. [CAP Theorem Tradeoff Analysis: Broker vs. Payment Engine](#-cap-theorem-tradeoff-analysis-broker-vs-payment-engine)
10. [Simplifications Relative to Apache Kafka & Raft](#-simplifications-relative-to-apache-kafka--raft)
11. [Build & Automated Test Suite Execution](#-build--automated-test-suite-execution)
...
## 💥 Chaos Testing & Split-Brain Invariant Proofs
...
2. **Leader Crash Test**:
   - Leader killed while writing.
   - New leader elected in $< 50\,\text{ms}$ with zero acknowledged message loss.
...
1. **Leader Election Coordination**:
   ...
   - *This Implementation*: `LeaderElector` and `ClusterController` implement an atomic in-memory coordinator evaluating the same ISR membership and highest LEO selection rules.
2. **Network Protocol**:
   - *Kafka*: Custom binary TCP wire protocol with zero-copy page cache transfers (`sendfile`).
   - *This Implementation*: High-concurrency Java concurrent primitives and thread-safe commit logs.
3. **Storage Tiering**:
   - *Kafka*: Segmented `.log` and `.index` files on persistent NVMe/SSD storage.
   - *This Implementation*: In-memory indexed commit logs with offset binary searching.
